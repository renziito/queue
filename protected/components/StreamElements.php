...
class StreamElements {
    ...
    protected function sendRequest($method, $endpoint, $params = array(), $isUpload = false) {
        ...
        if (is_array($this->apiLimits['global'])) {
            if ($this->apiLimits['global']['reset'] > new DateTime() && !$this->apiLimits['global']['remaining'])
                return false;
        }
    ...
    public function getInfo() {
        ...
        $this->profile = $res;
        if (!isset($this->profile["username"]) || !isset($this->profile["_id"]))
            return false;
